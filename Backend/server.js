...
// Referencing this ensures pkg's static analyzer scans scriptRegistry.js (it only
// bundles scripts reachable via require() from the entry point) - see that file for
// why these path.join() calls exist. The returned paths aren't used here; this call
// exists purely so the module is loaded and its literal path references get scanned.
require('./utils/scriptRegistry').registerDynamicallyInvokedScripts();
...
// Serve React build files. __dirname-relative on purpose, in BOTH dev and
// packaged mode: pkg's runtime fs-patching makes this transparently read
// from the embedded snapshot when packaged, the same way it reads from real
// disk in dev - no separate sibling folder needed for the exe to be truly
// single-file. That only works because the files are actually IN the
// snapshot in the first place; see frontendAssetRegistry.js (required
// below) for why that previously wasn't reliable and how it's fixed now.
const frontendBuildPath = path.join(__dirname, '../frontend/build');
app.use(express.static(frontendBuildPath));

// Referencing this ensures pkg's static analyzer scans frontendAssetRegistry.js
// (regenerated by scripts/generate-frontend-registry.js after every frontend
// build) - see that file for why. Not used here beyond that side effect. Wrapped
// because it's generated (gitignore-free but rewritten every build) - a fresh
// clone running dev mode before ever building the frontend shouldn't hard-crash
// over a module that only matters for the packaged exe.
try {
    require('./utils/frontendAssetRegistry').registerFrontendAssets();
} catch (err) {
    console.warn('frontendAssetRegistry.js not found - run npm run build first if packaging the exe.');
}
